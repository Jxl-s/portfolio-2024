...
export default function Decoration() {
	...
	const coffeeMaterial = useMemo(() => {
		const perlinAsset = getAsset<THREE.Texture>("perlin");
		perlinAsset.wrapS = THREE.RepeatWrapping;
		perlinAsset.wrapT = THREE.RepeatWrapping;

		return new CoffeeMaterial({
			side: THREE.DoubleSide,
			transparent: true,
			depthWrite: false,
			uniforms: {
				uTime: { value: 0 },
				uPerlinTexture: { value: perlinAsset },
			},
		});
	}, []);
